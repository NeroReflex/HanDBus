use std::{
    error::Error, ffi::CString, path::Path, time::{Duration, Instant}
};
...
impl Driver {
    pub fn new(udevice: UdevDevice) -> Result<Self, Box<dyn Error + Send + Sync>> {
        ...
        if !Path::new(path.as_str()).exists() {
            return Err(format!("Device '{}' does not have a devnode: a valid OrangePi NEO Controller has one", udevice.name()).into());
        }
        ...
        if info.vendor_id() != VID || info.product_id() != PID {
            return Err(format!("Device '{}' is not a OrangePi NEO Controller", udevice.name()).into());
        }
