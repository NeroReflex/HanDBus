...
impl Manager {
            ...
            match cmd {
                ...
                ManagerCommand::DeviceAdded { device } => {
                    ...
                    if let Err(e) = self.on_device_added(device).await {
                        log::error!("Error adding device '{} ({})': {}", dev_name, dev_sysname, e.to_string());
                    }
                }
                ManagerCommand::DeviceRemoved { device } => {
                    if let Err(e) = self.on_device_removed(device).await {
                        log::error!("Error removing device: {}", e.to_string());
                    }
                }
    ...
    pub async fn run(&mut self) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        let _ = tokio::join!(
            ...
            Self::watch_hidraw_devices(cmd_tx_hidraw, &mut watcher_rx),
            // Create a DBus interface
            Self::listen_on_dbus(dbus_for_listen_on_dbus, tx_for_listen_on_dbus),
            // Manage events generated by other tasks
            self.events_loop()
        );
    ...
    async fn on_device_added(&mut self, device: UdevDevice) -> Result<(), Box<dyn Error>> {
        ...
        if sys_name.is_empty() {
            log::warn!("Device discarded for missing sysname: {} at {}", device.name(), device.syspath());
            return Ok(());
        }
        ...
        match subsystem.as_str() {
            "input" => {
                if device.devnode().is_empty() {
                    log::warn!("Event device discarded for missing devnode: {} at {}", device.name(), device.sysname());
                    return Ok(());
                }

                log::debug!("Event device added: {} ({})", device.name(), device.sysname());
                ...
                if device.is_virtual() {
                    ...
                    if !is_bluetooth {
                        log::debug!("{dev_path} is virtual, skipping consideration.");
                        return Ok(());
                    }
                    log::debug!("{dev_path} is a virtual device node for a bluetooth device. Treating as real.")
                } else {
                    log::trace!("{dev_path} is a real device.");
                }
            ...
            "hidraw" => {
                if device.devnode().is_empty() {
                    log::warn!("Hidraw device discarded for missing devnode: {} at {}", device.name(), device.sysname());
                    return Ok(());
                }

                log::debug!("Hidraw device added: {} ({})", device.name(), device.sysname());
            ...
            "iio" => {
                if device.devnode().is_empty() {
                    log::warn!("IIO device discarded for missing devnode: {} at {}", device.name(), device.sysname());
                    return Ok(());
                }
    ...
    fn watch_iio_devices(cmd_tx: mpsc::Sender<ManagerCommand>) -> tokio::task::JoinHandle<Result<(), Box<dyn Error + std::marker::Send + Sync>>> {
            ...
            loop {
                ...
                for event in monitor.iter() {
                    ...
                    match action.to_str().unwrap() {
                        "add" => {
                            log::debug!("Got udev add action for {}", device.name());
                            cmd_tx.blocking_send(ManagerCommand::DeviceAdded {
                                device: device.into(),
                            })?;
                        }
                        "remove" => {
                            log::debug!("Got udev remove action for {}", device.name());
                            cmd_tx.blocking_send(ManagerCommand::DeviceRemoved {
                                device: device.into(),
                            })?;
                        }
                        _ => {
                            log::trace!("Unhandled udev action: {action:?}");
                        }
                    }
                }
                std::thread::sleep(Duration::from_millis(10));
    ...
    async fn watch_hidraw_devices(cmd_tx: mpsc::Sender<ManagerCommand>, watcher_rx: &mut mpsc::Receiver<WatchEvent>) -> () {
    ...
    async fn discover_devices(
        ...
        for device in devices {
            ...
            // If you end up with a SendErr here it might also mean there is a second instance running
            manager_tx
    ...
    async fn listen_on_dbus(dbus: Connection, tx: mpsc::Sender<ManagerCommand>) -> tokio::task::JoinHandle<()> {
