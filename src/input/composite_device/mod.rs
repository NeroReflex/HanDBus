...
use crate::{
    ...
    input::{
        ...
        source::{evdev::EventDevice, hidraw::HidRawDevice, iio::IioDevice, led::LedDevice, SourceDevice},
    },
    ...
    pub async fn run(
        ...
        for (path, target) in targets.iter() {
            if let Err(e) = target.set_composite_device(self.client()).await {
                return Err(
                    format!("Failed to set composite device for target device: {}", e.to_string()).into(),
                );
            }
            ...
            let caps = match target.get_capabilities().await {
                Ok(caps) => caps,
                Err(e) => {
                    return Err(format!("Failed to get target capabilities: {}", e.to_string()).into());
                }
            };
                        ...
                        let profile = match DeviceProfile::from_yaml(profile) {
                            ...
                            Err(e) => {
                                if let Err(er) = sender.send(Err(e.to_string().into())).await {
                                    log::error!("Failed to send failed to load profile: {}", er.to_string());
                                }
                                continue;
                            }
                        };
