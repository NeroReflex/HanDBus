...
use crate::{config::LedFixedColor, input::source::{SourceInputDevice, SourceOutputDevice}, udev::device::UdevDevice};
use thiserror::Error;
...
impl MultiColorChassis {
    ...
    /// device information
    pub fn new(device_info: UdevDevice, fixed_color: Option<LedFixedColor>) -> Result<Self, Box<dyn Error + Send + Sync>> {
        let multi_intensity_path = PathBuf::from(device_info.syspath().as_str()).join("multi_intensity");
        if !multi_intensity_path.exists() {
            return Err(Box::new(MultiColorChassisError::MultiIntensityNotFound(multi_intensity_path)))
        }

        let multi_index = PathBuf::from(device_info.syspath().as_str()).join("multi_index");
        let contents = std::fs::read_to_string(multi_index).map_err(|err| Box::new(MultiColorChassisError::MultiIndexError(err)))?;
        let multi_index_strings = contents.split_whitespace().into_iter().map(|str| String::from(str)).collect::<Vec<String>>();
        ...
        for idx in multi_index_strings.iter() {
            match idx.as_str() {
                ...
                _ => return Err(Box::new(MultiColorChassisError::UnsupportedIndexType(idx.clone())))
            }
        }

        let result = Self { device_info, fixed_color, multi_intensity_path, multi_index_map };
    ...
    fn write_color(&self, r: u8, g: u8, b: u8) -> Result<(), Box<dyn Error + Send + Sync>> {
        let contents = std::fs::read_to_string(self.multi_intensity_path.as_path()).map_err(|err| Box::new(MultiColorChassisError::MultiIndexError(err)))?;
        let contents = (self.multi_index_map.iter().zip(contents.split_whitespace().into_iter()).map(|(index_type, index_value)| 
            match index_type {
                IndexType::RGB => (((r as u32) << 0u32) | ((g as u32) << 8u32) | ((b as u32) << 16u32)).to_string(),
                _ => String::from(index_value)
            }
        ).collect::<Vec<String>>()).join(" ");
        Ok(std::fs::write(self.multi_intensity_path.as_path(), contents).map_err(|err| Box::new(MultiColorChassisError::MultiIntensityUpdateError(err)))?)
    }
}
