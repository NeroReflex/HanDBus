...
pub async fn get_device(path: String) -> Result<Device, Box<dyn Error>> {
    ...
    match output.devnum() {
        Some(val) => device.number = val,
        None => {}
    };

    match output.subsystem() {
        Some(val) => device.subsystem = String::from(val.to_string_lossy()),
        None => {}
    };

    match output.devtype() {
        Some(val) => device.device_type = String::from(val.to_string_lossy()),
        None => {}
    };

    match output.devnode() {
        Some(val) => device.node = String::from(val.to_string_lossy()),
        None => {}
    };

    match output.driver() {
        Some(val) => device.driver = String::from(val.to_string_lossy()),
        None => {}
    };
    ...
    // TODO: if it is useful in the future add network_index here
    Ok(device)
}
